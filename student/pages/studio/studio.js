...
Page({
  ...
  onLoad: function (options) {
    this.setData({descOnly : options.descOnly,jscode:options.jscode});
    let that = this;
    wx.request({
    ...
    let lics = [];
    wx.request({
      url: app.globalData.globalUrl.getStudioImg,
      success: function (res) {
  ...
  removeImage(e) {
    ...
    let arr = []
    wx.request({
      url: app.globalData.globalUrl.getStudioImg+'/'+id,
      method:'delete',
      success: (res)=> {
        if (type == 1) {
...
join:function(){
  wx.request({
    url: 'http://doit.svte.cn/api/v1/studio/' + app.globalData.studioId+'/students',
    method:'post',
    data: {
      "name": "新加入学员",
      "gender": 1,
      "account": this.data.jscode,
      "birthdate": "2000-01-01"
    },
    success:data=>{
      wx.showToast({
        title: '申请成功，请稍后补充详细信息',
      })
    }
  })
},
